...
const prepareSeriesTidy = (data, filters, variableCol, xCol, yCol, colors) => {
  ...
  const newData = f(data, filters);

  return getUnique(data, variableCol).map((v, iVar) => {
    hcData = [];
    const variableSeries = newData.filter((row) => row[variableCol] == v);
...
const prepareSeriesNonTidyUnits = (
  data,
  filters,
  unitsCurrent,
  baseUnits,
...
const seriesNonTidy = prepareSeriesNonTidy(
  data2,
  nglFilter,
  (valueVars = ["Pipeline", "Marine", "Railway", "Truck"]),
  (xCol = "Period"),
  nglColors
);
